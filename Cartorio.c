...
#include <stdlib.h> //biblioteca de alocação de espaço em memória p/ arrays...
...
int registrar() //Função responsável por cadastrar os usuários no sistema
{
	...
	file = fopen(arquivo, "a"); // file abra a função arquivo e atualiza ela
	...
	file = fopen(arquivo, "a"); //file abre a função aquivo e "a" atualiza ela
...
int consultar()
{
	...
	if(file ==NULL)
	{
		printf("Não foi possivel abrir seu arquivo, não localizado! \n");
	}
...
int main()
{
	int opcao=0; //definindo as variáveis (opção q é do tipo inteiro e recebe um valor igual a 0 nesse primeiro momento)
	int laco=1;
	...
	for(laco=1;laco=1;)
	{
		...
		scanf("%d", &opcao); //armazenando a escolha. Pc escanie a tela e oq o usuário escolher atribua a variável opcao
